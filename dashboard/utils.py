...
def get_wait_time_distribution_custom(start_time, end_time, bin_size=300, exclude_outliers=False, base_qs=None):
    if base_qs is None:
        base_qs = PersonSession.objects.all()
    qs = base_qs.filter(enter_timestamp__gte=start_time, enter_timestamp__lte=end_time).exclude(exit_timestamp__isnull=True).filter(duration_seconds__gt=1)
    if exclude_outliers:
        qs = _exclude_outliers_qs(qs, 'duration_seconds')
    if not qs.exists():
        return []
...
def get_top_longest_waits_custom(start_time, end_time, top_n=10, base_qs=None):
    if base_qs is None:
        base_qs = PersonSession.objects.all()
    qs = base_qs.filter(enter_timestamp__gte=start_time, enter_timestamp__lte=end_time).exclude(exit_timestamp__isnull=True).order_by('-duration_seconds')[:top_n]
    return list(qs)

def get_arrivals_by_day_of_week_custom(start_time, end_time, base_qs=None):
    if base_qs is None:
        base_qs = PersonSession.objects.all()
    qs = base_qs.filter(enter_timestamp__gte=start_time, enter_timestamp__lte=end_time).annotate(dw=ExtractWeekDay('enter_timestamp')).values('dw').annotate(count=Count('id')).order_by('dw')
...
def get_time_of_day_pattern_custom(start_time, end_time, base_qs=None):
    if base_qs is None:
        base_qs = PersonSession.objects.all()
    qs = base_qs.filter(enter_timestamp__gte=start_time, enter_timestamp__lte=end_time).annotate(hh=ExtractHour('enter_timestamp')).values('hh').annotate(count=Count('id')).order_by('hh')
...
def get_wait_distribution_by_dow_custom(start_time, end_time, base_qs=None):
    if base_qs is None:
        base_qs = PersonSession.objects.all()
    qs = base_qs.filter(enter_timestamp__gte=start_time, enter_timestamp__lte=end_time).exclude(exit_timestamp__isnull=True).filter(duration_seconds__gt=1).annotate(dw=ExtractWeekDay('enter_timestamp'))
...
def get_wait_distribution_by_hour_custom(start_time, end_time, base_qs=None):
    if base_qs is None:
        base_qs = PersonSession.objects.all()
    qs = base_qs.filter(enter_timestamp__gte=start_time, enter_timestamp__lte=end_time).exclude(exit_timestamp__isnull=True).filter(duration_seconds__gt=1).annotate(hh=ExtractHour('enter_timestamp'))
